...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        try:
            # Check if requesting session check or vault load
            path = self.path
            if path.startswith('/api/load-vault?check_session'):
                # Return session status without loading vault
                response_data = {"session_active": True, "vault_loaded": False}
            else:
                # Load core enforcement files only (smart loading)
                memory_content = load_core_enforcement()
                response_data = {
                    "success": True,
                    "memory": memory_content,
                    "vault_loaded": True,
                    "token_estimate": len(memory_content.split()) * 1.3  # Rough token estimate
                }
...
def load_core_enforcement():
    """Load essential enforcement files for session-based usage"""
    ...
    memory_content = "=== SITEMONKEYS BUSINESS INTELLIGENCE VAULT ===\n"
    memory_content += "=== LOADED FOR SESSION-BASED VALIDATION ===\n\n"
    
    try:
        # Load ESSENTIAL business intelligence files only
        essential_files = [
            ("VAULT_MEMORY_FILES", [
        ...
        for folder_name, files in essential_files:
            memory_content += load_folder_files(drive_service, vault_folder_id, folder_name, files)
        
        # Add session context
        memory_content += "\n=== SESSION CONTEXT ===\n"
        memory_content += "PURPOSE: Critical SiteMonkeys business validation and launch preparation\n"
        memory_content += "ROLE: Zero-failure business intelligence system\n"
        memory_content += "BEHAVIOR: Follow founder's directives above all else - no generic advice\n"
        memory_content += "FOCUS: Real-world survivability, margin protection, scalability validation\n\n"
        
        # Add available resources
        memory_content += "=== ADDITIONAL VAULT RESOURCES AVAILABLE ===\n"
        memory_content += "Legal Documents (NDAs, Terms, Privacy Policy)\n"
        memory_content += "Contractor Handoff Materials\n"
        memory_content += "Implementation Roadmaps and Checklists\n"
        memory_content += "AI Tuning and QA Protocols\n"
        memory_content += "Request specific files: 'Load [category] files for [specific need]'\n\n"
        
        return memory_content
        
    except Exception as e:
        raise Exception(f"Core enforcement loading failed: {e}")
