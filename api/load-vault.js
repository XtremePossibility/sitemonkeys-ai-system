...
async function storeVaultInKv(vaultData) {
  try {
    ...
    const response = await axios.post(`${kvUrl}/set/sitemonkeys_vault_v2`, requestData, {
      headers,
      timeout: 30000,
    });
...
async function getVaultFromKv() {
  try {
    ...
    const response = await axios.get(`${kvUrl}/get/sitemonkeys_vault_v2`, {
      headers,
      timeout: 30000,
    });
    ...
    console.log(
      `📥 KV Retrieval response length: ${response.data ? JSON.stringify(response.data).length : 0}`,
    );
    ...
    if (response.status === 200) {
      try {
        ...
        if (typeof data === 'object' && data.compressed) {
          ...
          console.log(
            `📦 Decompressed: ${data.compressed_size || 0} → ${data.original_size || 0} bytes`,
          );
          return result;
        } else {
...
app.all('/api/load-vault', async (req, res) => {
  try {
    ...
    if (isRefresh) {
      ...
      console.log(
        `📊 Vault refresh complete: ${tokenCount} tokens, ${loadedFolders.length} folders`,
      );
