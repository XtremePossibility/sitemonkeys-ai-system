...
export class SemanticAnalyzer {
  ...
  async initialize() {
    try {
      ...
      const intentPhrases = {
        ...
        information_sharing: "I want to tell you about this. Here's what happened. This is important to know."
      };
      
      // Domain representative phrases
      const domainPhrases = {
        business: "revenue growth, market strategy, customer acquisition, business model, profitability, scaling operations",
        technical: "software development, coding, system architecture, API integration, debugging, technical implementation",
        personal: "relationships, family matters, personal growth, social connections, life decisions, personal experiences",
        health: "medical concerns, wellness, fitness, symptoms, healthcare, mental health, physical wellbeing",
        financial: "money management, investments, budgeting, financial planning, savings, debt, income",
        creative: "artistic projects, creative writing, design work, creative problem solving, artistic expression",
  ...
  async #calculateComplexity(query, queryEmbedding) {
    try {
      ...
      const conjunctions = (query.match(/\b(and|or|but|however|therefore|because)\b/gi) || []).length;
      factors.interdependencies = Math.min((questionMarks + conjunctions) / 5, 1.0);
      
      // Factor 3: Ambiguity
      const ambiguousTerms = (query.match(/\b(maybe|might|could|possibly|probably|somewhat|kind of)\b/gi) || []).length;
      ...
      if (this.domainEmbeddings && this.domainEmbeddings.technical) {
        const technicalSimilarity = this.#cosineSimilarity(queryEmbedding, this.domainEmbeddings.technical);
  ...
  #detectContextSignals(query, queryEmbedding, context) {
    try {
      ...
      const memoryIndicators = /\b(remember|recall|told you|mentioned before|we discussed|last time)\b/i;
  ...
  #assessReasoningNeeds(query, queryEmbedding, intentResult) {
    try {
      ...
      const calculationPatterns = /\b(calculate|compute|how much|total|sum|average|cost|price|\d+)\b/i;
      ...
      const creativityPatterns = /\b(create|design|imagine|innovative|new idea|brainstorm|creative)\b/i;
      const requiresCreativity = creativityPatterns.test(query) || 
  ...
  getStats() {
    return {
      ...
      cacheHitRate: this.stats.totalAnalyses > 0
        ...
        : 0,
      avgCostPerQuery: this.stats.totalAnalyses > 0
