...
class LearningEngine {
  ...
  async recordExperience({ 
    ...
    console.log(`✅ Experience recorded and learned from - Total experiences: ${this.experienceDatabase.length}`);
  ...
  async learnConfidenceAccuracy(experience) {
    ...
    const rangeData = pattern.successful_patterns.get(confidenceRange) || { predictions: 0, successes: 0 };
  ...
  inferSatisfactionFromMetrics(outputAnalysis) {
    const score = (outputAnalysis.confidence + outputAnalysis.trust_score + 
                  outputAnalysis.precision_score + outputAnalysis.reliability_score) / 4;
    return score;
  }
  ...
  identifyMostEffectiveApproaches() {
    ...
    this.learningPatterns.get('reasoning_effectiveness').successful_patterns.forEach((data, approach) => {
      if (data.total_uses > 5) { // Minimum sample size
        const score = (data.avg_confidence + data.avg_trust_score + 
                      data.avg_strategic_depth/5 + data.novel_insights_rate) / 4;
  ...
  identifyImprovementOpportunities() {
    ...
    const lowConfidenceRate = confidencePattern.confidence_levels
      .filter(c => c.predicted < 0.7).length / confidencePattern.confidence_levels.length;
  ...
  updateGlobalImprovementMetrics() {
    ...
    if (recentExperiences.length === 100) {
      const recentAvgConfidence = recentExperiences.reduce((sum, exp) => 
        sum + exp.output_analysis.confidence, 0) / 100;
      
      const previousAvgConfidence = this.experienceDatabase
        .slice(-200, -100)
