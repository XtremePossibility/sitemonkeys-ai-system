...
export function calculateRiskSeverity(triggers, config, messageLower) {
  ...
  if (
    config.risks.includes('cash_runway_depletion') ||
    config.risks.includes('business_survival')
  ) {
    severity = 'critical';
  }
...
export function applyProtectiveIntelligence(
  response,
  originalMessage,
  expertDomain,
  conversationHistory,
  ...
  const initiativeResponse = generateInitiativeBasedResponse(
    protectiveAlerts,
    solutionOpportunities,
    originalMessage,
  );
...
export function containsProtectiveAnalysis(response) {
  ...
  return protectiveIndicators.some((indicator) => response.toLowerCase().includes(indicator));
}
