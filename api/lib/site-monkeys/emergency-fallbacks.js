...
- Provide actionable insights`,
  ...
  system_responses: {
    system_error_response:
      "I encountered a system processing error, but I'm still operational and ready to help with your business validation needs.",
  },
};
...
function validateVaultStructure(vaultData) {
  try {
    ...
    if (validationPassed) {
      console.log(
        `✅ Vault validation passed. Found business content: ${termCount}/${contentTerms.length} terms`,
      );
    } else {
      console.warn(
        `⚠️ Vault validation failed. Found: ${termCount}/${contentTerms.length} terms, Length: ${vaultData.length}`,
      );
    }
