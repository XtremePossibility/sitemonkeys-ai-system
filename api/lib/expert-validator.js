...
export function checkExpertIndicators(response, expertDomain) {
  ...
  if (expertDomain === 'financial_analysis') {
    indicators.uses_specific_frameworks = /cash flow|roi|margin|break-even|runway|scenario/i.test(response);
    indicators.uses_expert_terminology = /ebitda|cac|ltv|burn rate|runway|working capital/i.test(response);
  } else if (expertDomain === 'business_strategy') {
    indicators.uses_specific_frameworks = /competitive|market|positioning|differentiation|value prop/i.test(response);
    indicators.uses_expert_terminology = /moat|traction|product-market fit|go-to-market/i.test(response);
  }
  
  // Universal expert indicators
  indicators.demonstrates_deep_knowledge = response.length > 500 && /because|since|due to|given that/i.test(response);
  indicators.identifies_blind_spots = /haven't considered|might miss|blind spot|overlooked|not seeing/i.test(response);
  indicators.provides_professional_insights = /in my experience|best practice|typically|usually|pattern/i.test(response);
  indicators.anticipates_problems = /risk|challenge|potential issue|concern|caution|watch out/i.test(response);
  indicators.references_best_practices = /best practice|industry standard|proven approach|established method/i.test(response);
  indicators.shows_industry_awareness = /market trend|industry|competitive|benchmark|standard/i.test(response);
...
export function performQualityChecks(response, originalMessage) {
  const checks = {
    has_confidence_scoring: /confidence:|confidence\s+(high|medium|low)/i.test(response),
    flags_assumptions: /assum|presuppos|given that|based on the idea/i.test(response),
    provides_alternatives: /alternative|instead|could also|another approach|option/i.test(response),
    identifies_risks: /risk|danger|concern|caution|potential issue/i.test(response),
    gives_next_steps: /next step|recommend|suggest|should|action/i.test(response),
    shows_reasoning: /because|since|due to|therefore|as a result/i.test(response),
...
export function detectDrift(response) {
  ...
  Object.entries(DRIFT_INDICATORS).forEach(([category, patterns]) => {
    const matches = patterns.filter(pattern => responseLower.includes(pattern.toLowerCase()));
    if (matches.length > 0) {
...
export function analyzeAssumptions(response) {
  const assumptionAnalysis = {
    assumptions_flagged: /assum|presuppos|given that|based on the idea/i.test(response),
  ...
  assumptionAnalysis.quality_score = calculateAssumptionQuality(assumptionAnalysis);
...
export function countAssumptions(response) {
  const assumptionWords = ['assume', 'assuming', 'presume', 'suppose', 'given that', 'if we assume'];
...
export function enforceExpertStandards(response, expertDomain, originalMessage) {
  const validation = validateExpertQuality(response, expertDomain, originalMessage);
  ...
  if (validation.expert_level < 70) {
    enforcedResponse = addExpertEnhancements(enforcedResponse, expertDomain, validation);
  }
  
  // Fix drift issues
  if (validation.drift_analysis.drift_detected) {
    enforcedResponse = correctDrift(enforcedResponse, validation.drift_analysis);
  }
  
  // Fix assumption issues
  if (validation.assumption_analysis.quality_score < 70) {
    enforcedResponse = improveAssumptionHandling(enforcedResponse, validation.assumption_analysis);
  }
...
export function correctDrift(response, driftAnalysis) {
  ...
  driftAnalysis.drift_patterns.forEach(pattern => {
    pattern.matches.forEach(match => {
      ...
      corrected = corrected.replace(new RegExp(match, 'gi'), getExpertReplacement(match));
    });
  });
...
export function getExpertReplacement(weakPhrase) {
  ...
  return replacements[weakPhrase.toLowerCase()] || 'Professional analysis indicates';
}
...
export function monitorSystemDrift(responses) {
  ...
  driftTracking.average_expert_level = 
    validations.reduce((sum, v) => sum + v.expert_level, 0) / validations.length;
  
  driftTracking.average_quality_score = 
    validations.reduce((sum, v) => sum + v.quality_score, 0) / validations.length;
  ...
  if (responses.length > 3) {
    ...
    const recentAvg = recent.reduce((sum, v) => sum + v.quality_score, 0) / recent.length;
    const earlierAvg = earlier.reduce((sum, v) => sum + v.quality_score, 0) / earlier.length;
