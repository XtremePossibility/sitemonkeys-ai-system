...
export function checkExpertIndicators(response, expertDomain) {
  ...
  if (expertDomain === 'financial_analysis') {
    indicators.uses_specific_frameworks = /cash flow|roi|margin|break-even|runway|scenario/i.test(
      response,
    );
    indicators.uses_expert_terminology = /ebitda|cac|ltv|burn rate|runway|working capital/i.test(
      response,
    );
  } else if (expertDomain === 'business_strategy') {
    indicators.uses_specific_frameworks =
      /competitive|market|positioning|differentiation|value prop/i.test(response);
    indicators.uses_expert_terminology = /moat|traction|product-market fit|go-to-market/i.test(
      response,
    );
  }
  ...
  indicators.identifies_blind_spots =
    /haven't considered|might miss|blind spot|overlooked|not seeing/i.test(response);
  ...
  indicators.references_best_practices =
    /best practice|industry standard|proven approach|established method/i.test(response);
...
export function countAssumptions(response) {
  ...
  return assumptionWords.filter((word) => response.toLowerCase().includes(word.toLowerCase()))
    .length;
}
