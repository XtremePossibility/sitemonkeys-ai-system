...
// CORRECTED IMPORTS - Using your actual class-based enforcement modules
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 🔧 ENFORCEMENT-FIRST SEQUENCE - ACTUALLY REPLACE RESPONSES
    ...
    const politicalResult = PoliticalGuardrails.guardPoliticalContent(response, message);
    ...
    if (politicalResult.political_intervention) {
      ...
      // Add mode fingerprint to political response
    ...
    const productValidation = ProductValidator.validateRecommendation(finalResponse, mode);
    console.log('🔍 Product validation:', { 
      passed: productValidation.validation_passed,
      evidence_strength: productValidation.evidence_strength,
      enforcement_actions: productValidation.enforcement_actions.length
    });
    
    if (!productValidation.validation_passed) {
      const structured = ProductValidator.generateStructuredRecommendation(finalResponse, productValidation);
      
      finalResponse = `RECOMMENDATION VALIDATION FAILED
...
Would you like me to research this topic more thoroughly to provide a properly validated recommendation?`;
    ...
    // Add mode fingerprint and debug info
    finalResponse += `\n\n🔍 [DEBUG] Mode: ${mode} | Vault: ${vaultStatus} | Security: ${securityPass ? 'PASS' : 'FAIL'}`;

    console.log('✅ All enforcement layers completed');
