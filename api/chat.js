    ...
    if (kvResponse.ok) {
      ...
      if (kvText && kvText !== 'null' && kvText.trim() !== '') {
        try {
          ...
          // FIXED: More lenient vault content validation
...
1. Clear conversation history to reduce context
...
systemPrompt += `MODE: Truth-General Focus on factual accuracy and personal clarity without business bias. `;

} else if (mode === ‘business_validation’) {
systemPrompt += `MODE: Business Validation Focus on business survival, financial reality, and conservative risk assessment. Always include SURVIVAL_RISK assessment in business analysis. `;
...
```
// FIXED: Vault injection logic - much more aggressive
...
CRITICAL INSTRUCTION: Use this vault intelligence extensively to inform responses about Site Monkeys operations, pricing, protocols, and business standards. Reference specific vault information directly. Do not use generic or fallback language when vault data is available.

`; console.log(`🎯 Vault injected into system prompt: ${vaultContent.length} characters`); } else { systemPrompt += `VAULT STATUS: Operating in fallback mode due to vault loading error.
...
`; console.log(`⚠️ Using fallback prompt - vault content: ${vaultContent ? vaultContent.length : 0} characters`);
}
}
