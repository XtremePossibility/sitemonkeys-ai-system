...
const ASSUMPTION_TRACKER = {
  health_thresholds: {
    staleness_days: 30,      // Flag if not validated in 30 days
    override_frequency: 3,    // Flag if overridden 3+ times in 14 days
    confidence_minimum: 0.70  // Flag if confidence drops below 70%
  },
...
function trackAssumptionOverride(assumption_name, original_value, overridden_value, reason) {
  if (ASSUMPTION_TRACKER.tracked_assumptions[assumption_name]) {
    ...
    // Reduce confidence score slightly with each override
...
// FIXED: Generate vault context if triggers found
function generateVaultContext(triggeredFrameworks, vaultLoaded) {
  ...
  triggeredFrameworks.forEach(framework => {
    ...
    // Map framework triggers to actual decision framework names
    switch(framework) {
      case 'pricing':
...
// FIXED: Generate Eli's analytical response
...
// FIXED: Generate Roxy's creative response
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Check assumption health and add warnings if needed
