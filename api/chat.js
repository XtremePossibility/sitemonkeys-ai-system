...
// AUTHENTIC RECOMMENDATION ENGINE - Truth over popularity
const RECOMMENDATION_ENGINE = {
  core_principles: [
    "NEVER recommend based on popularity alone",
    "ALWAYS prioritize what is best for the user's specific goal, budget, and constraints",
    "SURFACE multiple viable options with trade-offs, not just a single choice",
    "FLAG hype, marketing influence, or lack of evidence when relevant",
    "OFFER filtering based on user's stated priorities (cost, ease, performance)"
  ],
  
  option_framework: {
    cost_optimized: "Lowest cost that still meets quality threshold",
    mainstream: "Most widely used or proven in market",
    premium: "Top-tier/high-performance regardless of cost",
    unconventional: "Lesser-known or creative option that fits unique needs",
    situation_specific: "Tailored pick based on user's case"
  },
  
  response_voice: {
    tone: "Trusted Advisor — warm, honest, and user-protective",
    explanation: "I don't just list what's popular — I recommend what actually fits you.",
    structure: [
      "Understand what user is trying to achieve",
      "Describe how and why options were chosen", 
      "Make consequences of each choice transparent",
      "Encourage user to adjust filter or reprioritize if needed"
    ]
  },
  
  fallback_behavior: {
    insufficient_info: "I can't give you the right recommendation until I understand a bit more. Want to tell me your budget, constraints, or what matters most?",
    no_good_options: "None of the usual picks seem like a good match here. Want me to help explore alternatives that might work better?"
  }
};
...
- Never recommend based on popularity alone
- Show 3-5 meaningful options: cost-optimized, mainstream, premium, unconventional, situation-specific
- Always explain why each option was chosen and trade-offs involved
- Flag overhyped or misaligned popular choices
...
RECOMMENDATION INTELLIGENCE:
- Prioritize what's best for user's specific business situation over popular choices
- Present multiple viable options with clear trade-offs and consequences
- Flag solutions that are overhyped or misaligned with business needs
- Always explain why each option was selected and what it optimizes for
- Ask clarifying questions about budget, constraints, and priorities
...
RECOMMENDATION INTELLIGENCE:
- Always prioritize solutions that align with Site Monkeys business model and constraints
- Present options that optimize for the specific financial and operational parameters
- Flag any recommendations that could compromise business viability or brand positioning
- Explain trade-offs in context of Site Monkeys goals and limitations
- Consider Site Monkeys pricing tiers and target market when suggesting solutions
...
function runOptimizationEnhancer(params) {
  ...
  let complexityFlags = [];
  let recommendationImprovements = [];
  ...
  // Recommendation quality detection
  if (messageLC.includes('recommend') || messageLC.includes('suggest') || messageLC.includes('what should') || messageLC.includes('which') || messageLC.includes('best')) {
    recommendationImprovements.push("🎯 Recommendation Note: I focus on what's best for your specific situation, not what's most popular");
  }
  
  // Tool/software selection detection
  if (messageLC.includes('tool') || messageLC.includes('software') || messageLC.includes('platform') || messageLC.includes('service')) {
    recommendationImprovements.push("🔧 Tool Selection: I'll show you options across different priorities - cost, mainstream reliability, premium quality, and situation-specific fits");
  }
  
  // Build enhanced response
  let enhancedResponse = baseResponse;
  
  if (optimizationTips.length > 0 || complexityFlags.length > 0 || recommendationImprovements.length > 0) {
    ...
    recommendationImprovements.forEach(improvement => {
      enhancedResponse += `${improvement}\n`;
    });
  }
  ...
  return {
    ...
    recommendation_improvements: recommendationImprovements
  };
}
