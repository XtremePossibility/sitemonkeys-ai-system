// SITE MONKEYS AI - LEVEL 7 LOCKDOWN - ZERO-FAILURE ENFORCEMENT
const FULL_ENFORCEMENT_MODE = true;

import { trackApiCall, formatSessionDataForUI } from ‘./lib/tokenTracker.js’;

export default async function handler(req, res) {
res.setHeader(‘Access-Control-Allow-Origin’, ‘*’);
res.setHeader(‘Access-Control-Allow-Methods’, ‘POST, OPTIONS’);
res.setHeader(‘Access-Control-Allow-Headers’, ‘Content-Type’);

if (req.method === ‘OPTIONS’) {
res.status(200).end();
return;
}

if (req.method !== ‘POST’) {
res.status(405).json({ error: ‘Method not allowed’ });
return;
}

let vaultContent = ‘’;
let vaultTokens = 0;
let vaultStatus = ‘not_loaded’;
let vaultStructuralIntegrity = false;

try {
const { message, conversation_history = [], mode = ‘site_monkeys’, claude_requested = false } = req.body;

```
if (!message || typeof message !== 'string') {
  res.status(400).json({ error: 'Message is required and must be a string' });
  return;
}

console.log('Processing chat request in ' + mode + ' mode:', message.substring(0, 100));

// VAULT LOADING WITH STRUCTURAL VALIDATION
if (mode === 'site_monkeys') {
  try {
    const kv_url = process.env.KV_REST_API_URL;
    const kv_token = process.env.KV_REST_API_TOKEN;
    
    if (!kv_url || !kv_token) {
      if (FULL_ENFORCEMENT_MODE) {
        throw new Error('ENFORCEMENT FAILURE: KV environment variables required in full enforcement mode');
      }
      throw new Error('KV environment variables not configured');
    }
    
    const kvResponse = await fetch(kv_url + '/get/sitemonkeys_vault_v2', {
      headers: { 
        'Authorization': 'Bearer ' + kv_token,
        'Content-Type': 'application/json'
      }
    });
    
    if (kvResponse.ok) {
      const kvText = await kvResponse.text();
      
      if (kvText && kvText !== 'null' && kvText.trim() !== '') {
        let kvData;
        const kvWrapper = JSON.parse(kvText);
        
        if (kvWrapper.result) {
          kvData = JSON.parse(kvWrapper.result);
        } else if (kvWrapper.compressed) {
          try {
            const decompressed = atob(kvWrapper.data);
            kvData = JSON.parse(decompressed);
          } catch (decompError) {
            kvData = kvWrapper;
          }
        } else {
          kvData = kvWrapper;
        }
        
        if (kvData.vault_content && kvData.vault_content.length > 1000) {
          vaultContent = kvData.vault_content;
          vaultTokens = kvData.tokens || Math.ceil(vaultContent.length / 4);
          vaultStatus = 'loaded';
          
          // STRUCTURAL INTEGRITY VALIDATION
          vaultStructuralIntegrity = validateVaultStructure(vaultContent);
          
          if (FULL_ENFORCEMENT_MODE && !vaultStructuralIntegrity) {
            console.log('ENFORCEMENT WARNING: Vault structure validation failed');
          }
          
        } else {
          if (FULL_ENFORCEMENT_MODE) {
            throw new Error('ENFORCEMENT FAILURE: Vault content insufficient for full enforcement mode');
          }
          throw new Error('Vault content missing or insufficient');
        }
      } else {
        if (FULL_ENFORCEMENT_MODE) {
          throw new Error('ENFORCEMENT FAILURE: Empty KV response in full enforcement mode');
        }
        throw new Error('KV returned empty data');
      }
    } else {
      if (FULL_ENFORCEMENT_MODE) {
        throw new Error('ENFORCEMENT FAILURE: KV API error ' + kvResponse.status + ' in full enforcement mode');
      }
      throw new Error('KV API error: ' + kvResponse.status);
    }
    
  } catch (vaultError) {
    if (FULL_ENFORCEMENT_MODE && vaultError.message.includes('ENFORCEMENT FAILURE')) {
      return res.status(500).json({
        response: 'FULL ENFORCEMENT MODE FAILURE: ' + vaultError.message + '\n\nSystem cannot operate without complete vault intelligence in enforcement mode. Please refresh vault and try again.',
        mode_active: mode,
        vault_status: { loaded: false, tokens: 0, enforcement_failure: true },
        error: 'Full enforcement mode requirements not met'
      });
    }
    
    vaultStatus = 'failed';
    vaultContent = 'VAULT_ERROR: ' + vaultError.message + '\n\nFALLBACK SITE MONKEYS BUSINESS LOGIC:\n- Premium web development services starting at $697\n- Business validation and truth-first analysis\n- Zero-compromise professional standards\n- Client success and quality assurance focus';
    vaultTokens = Math.ceil(vaultContent.length / 4);
  }
}

// PERSONALITY DETERMINATION WITH ENFORCEMENT CHECKS
let personality = claude_requested ? 'claude' : determinePersonality(message, mode);

if (claude_requested) {
  if (FULL_ENFORCEMENT_MODE && vaultStatus !== 'loaded') {
    return res.status(200).json({
      response: 'CLAUDE BLOCKED IN FULL ENFORCEMENT MODE: Claude requires complete vault intelligence. Vault status: ' + vaultStatus + '. Please refresh vault or use Eli/Roxy.',
      mode_active: mode,
      vault_status: { loaded: false, tokens: vaultTokens, enforcement_blocked: true },
      claude_blocked: true,
      enforcement_reason: 'vault_required'
    });
  }
  
  const estimatedTokens = Math.ceil((buildSystemPrompt(mode, personality, vaultContent).length + message.length) / 4) + 500;
  const estimatedCost = (estimatedTokens * 0.015) / 1000;
  
  if (estimatedCost > 0.50) {
    return res.status(200).json({
      response: 'CLAUDE COST LIMIT EXCEEDED: $' + estimatedCost.toFixed(4) + ' exceeds $0.50 limit. Use Eli or Roxy instead.',
      mode_active: mode,
      vault_status: { loaded: vaultStatus === 'loaded', tokens: vaultTokens },
      claude_blocked: true,
      cost_reason: 'limit_exceeded'
    });
  }
}

const systemPrompt = buildSystemPrompt(mode, personality, vaultContent);
const fullPrompt = buildFullPrompt(systemPrompt, message, conversation_history);
const apiResponse = await makeRealAPICall(fullPrompt, personality);

let promptTokens, completionTokens;

if (personality === 'claude') {
  promptTokens = apiResponse.usage?.input_tokens || Math.ceil(fullPrompt.length / 4);
  completionTokens = apiResponse.usage?.output_tokens || Math.ceil(apiResponse.response.length / 4);
} else {
  promptTokens = apiResponse.usage?.prompt_tokens || Math.ceil(fullPrompt.length / 4);
  completionTokens = apiResponse.usage?.completion_tokens || Math.ceil(apiResponse.response.length / 4);
}

const trackingResult = trackApiCall(personality, promptTokens, completionTokens, vaultTokens);
const enforcedResponse = applySystemEnforcement(apiResponse.response, mode, vaultContent, vaultStatus);
const sessionData = formatSessionDataForUI();

res.status(200).json({
  response: enforcedResponse,
  mode_active: mode,
  vault_status: {
    loaded: vaultStatus === 'loaded',
    tokens: vaultTokens,
    status: vaultStatus,
    structural_integrity: vaultStructuralIntegrity
  },
  enforcement_applied: [
    'truth_enforcement_active',
    'confidence_scoring_enforced',
    'political_neutrality_enforced',
    mode === 'site_monkeys' && vaultStatus === 'loaded' ? 'vault_business_logic' : 'fallback_mode',
    'assumption_analysis_active',
    FULL_ENFORCEMENT_MODE ? 'full_enforcement_mode' : 'standard_mode'
  ],
  assumption_analysis: {
    detected: extractAssumptions(enforcedResponse),
    health_score: calculateAssumptionHealth(enforcedResponse)
  },
  security_pass: true,
  performance: {
    tokens_used: trackingResult.tokens_used,
    call_cost: trackingResult.call_cost,
    session_total: trackingResult.session_total,
    vault_tokens: vaultTokens,
    api_provider: personality === 'claude' ? 'anthropic' : 'openai',
    api_error: { fallback_used: false }
  },
  session_tracking: sessionData,
  personality_used: personality,
  enforcement_mode: FULL_ENFORCEMENT_MODE ? 'full' : 'standard'
});
```

} catch (error) {
console.error(‘Chat processing error:’, error);

```
res.status(500).json({
  response: 'SYSTEM ERROR: ' + error.message + (FULL_ENFORCEMENT_MODE ? '\n\nFULL ENFORCEMENT MODE: Error handling protocols active.' : '') + '\n\nSite Monkeys truth-first enforcement remains operational with ' + vaultTokens + ' vault tokens available.',
  mode_active: req.body.mode || 'site_monkeys',
  vault_status: { loaded: false, tokens: vaultTokens },
  enforcement_applied: ['error_recovery_active'],
  error: 'Chat processing failed - recovery mode active'
});
```

}
}

async function makeRealAPICall(prompt, personality) {
if (personality === ‘claude’) {
if (!process.env.ANTHROPIC_API_KEY) {
throw new Error(‘CLAUDE API KEY MISSING: Cannot process Claude requests without valid API key’);
}

```
try {
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + process.env.ANTHROPIC_API_KEY,
      'Content-Type': 'application/json',
      'x-api-version': '2023-06-01'
    },
    body: JSON.stringify({
      model: 'claude-3-sonnet-20240229',
      max_tokens: 1000,
      messages: [{ role: 'user', content: prompt }]
    })
  });

  if (!response.ok) {
    throw new Error('Claude API HTTP error: ' + response.status + ' ' + response.statusText);
  }

  const data = await response.json();
  
  // HARDENED CLAUDE RESPONSE PARSING
  let responseText = '';
  
  if (data.content && Array.isArray(data.content)) {
    if (data.content.length > 0 && data.content[0] && typeof data.content[0].text === 'string') {
      responseText = data.content[0].text;
    } else if (data.content.length > 0 && typeof data.content[0] === 'string') {
      responseText = data.content[0];
    } else {
      responseText = 'Claude response format error: content array malformed';
    }
  } else if (data.content && typeof data.content === 'string') {
    responseText = data.content;
  } else if (data.message && typeof data.message === 'string') {
    responseText = data.message;
  } else if (data.text && typeof data.text === 'string') {
    responseText = data.text;
  } else {
    responseText = 'Claude API response parsing failed: unrecognized format';
    console.error('Claude response structure:', JSON.stringify(data, null, 2));
  }
  
  return {
    response: responseText,
    usage: data.usage || {
      input_tokens: Math.ceil(prompt.length / 4),
      output_tokens: Math.ceil(responseText.length / 4)
    }
  };
  
} catch (claudeError) {
  console.error('Claude API call failed:', claudeError);
  throw new Error('Claude API failure: ' + claudeError.message);
}
```

}

if (!process.env.OPENAI_API_KEY) {
throw new Error(‘OPENAI API KEY MISSING: Cannot process Eli/Roxy requests without valid API key’);
}

try {
const response = await fetch(‘https://api.openai.com/v1/chat/completions’, {
method: ‘POST’,
headers: {
‘Authorization’: ’Bearer ’ + process.env.OPENAI_API_KEY,
‘Content-Type’: ‘application/json’
},
body: JSON.stringify({
model: ‘gpt-3.5-turbo’,
messages: [{ role: ‘user’, content: prompt }],
max_tokens: 1000,
temperature: personality === ‘eli’ ? 0.3 : 0.7
})
});

```
if (!response.ok) {
  throw new Error('OpenAI API HTTP error: ' + response.status + ' ' + response.statusText);
}

const data = await response.json();

if (!data.choices || !Array.isArray(data.choices) || data.choices.length === 0) {
  throw new Error('OpenAI response format error: no choices array');
}

if (!data.choices[0].message || !data.choices[0].message.content) {
  throw new Error('OpenAI response format error: no message content');
}

return {
  response: data.choices[0].message.content,
  usage: data.usage || {
    prompt_tokens: Math.ceil(prompt.length / 4),
    completion_tokens: Math.ceil(data.choices[0].message.content.length / 4)
  }
};
```

} catch (openaiError) {
console.error(‘OpenAI API call failed:’, openaiError);
throw new Error(’OpenAI API failure: ’ + openaiError.message);
}
}

function validateVaultStructure(vaultContent) {
if (!vaultContent || typeof vaultContent !== ‘string’) {
return false;
}

const requiredMarkers = [
‘=== SITEMONKEYS BUSINESS VALIDATION VAULT ===’,
‘=== LIVE VAULT FOLDERS LOADED’,
‘— FOLDER:’
];

const hasRequiredMarkers = requiredMarkers.every(marker =>
vaultContent.includes(marker)
);

const folderCount = (vaultContent.match(/— FOLDER:/g) || []).length;
const readableContentRatio = vaultContent.replace(/[^a-zA-Z\s]/g, ‘’).length / vaultContent.length;

return hasRequiredMarkers && folderCount >= 2 && readableContentRatio > 0.6;
}

function buildSystemPrompt(mode, personality, vaultContent = ‘’) {
let systemPrompt = ’You are ’ + (personality === ‘eli’ ? ‘Eli (analytical)’ : personality === ‘roxy’ ? ‘Roxy (strategic)’ : ‘Claude’) + ‘, Site Monkeys AI.\n\n’;

systemPrompt += ‘TRUTH-FIRST REQUIREMENTS:\n- Every factual claim MUST include confidence level (High/Medium/Low/Unknown)\n- When uncertain, say “I do not know” rather than guess\n- Label speculation as “SPECULATION:” or “HYPOTHESIS:”\n- Mark assumptions when relevant\n- NEVER tell anyone who to vote for\n\n’;

if (mode === ‘site_monkeys’) {
if (vaultContent && vaultContent.length > 1000) {
systemPrompt += ‘SITE MONKEYS BUSINESS INTELLIGENCE VAULT:\n’ + vaultContent + ‘\n\nCRITICAL INSTRUCTION: Use this vault intelligence extensively. Begin responses with “Based on the Site Monkeys vault protocols” when referencing vault information. Reference specific pricing, policies, and procedures from the vault.\n\n’;
} else {
systemPrompt += ‘FALLBACK MODE: Premium web development starting at $697, business validation focus, truth-first analysis.\n\n’;
}
}

return systemPrompt;
}

function buildFullPrompt(systemPrompt, message, conversationHistory) {
let fullPrompt = systemPrompt;

if (conversationHistory.length > 0) {
fullPrompt += ‘RECENT CONVERSATION:\n’;
conversationHistory.slice(-2).forEach(msg => {
fullPrompt += (msg.role === ‘user’ ? ’Human: ’ : ’Assistant: ’) + msg.content + ‘\n’;
});
}

fullPrompt += ’\nCURRENT REQUEST:\nHuman: ’ + message + ‘\n\nResponse:’;
return fullPrompt;
}

function applySystemEnforcement(response, mode, vaultContent, vaultStatus) {
let enforcedResponse = response;

// STRENGTHENED CONFIDENCE SCORING ENFORCEMENT
const hasValidConfidence = /CONFIDENCE:\s*(High|Medium|Low|Unknown)/i.test(response);
const hasMalformedConfidence = /CONFIDENCE:/i.test(response) && !hasValidConfidence;
const hasFactualClaims = containsFactualClaims(response);

if (hasMalformedConfidence) {
enforcedResponse = enforcedResponse.replace(/CONFIDENCE:.*$/m, ‘CONFIDENCE: Medium (AI processing - corrected)’);
} else if (!hasValidConfidence && hasFactualClaims) {
enforcedResponse += ‘\n\nCONFIDENCE: Medium (AI processing)’;
}

// POLITICAL NEUTRALITY ENFORCEMENT
const politicalKeywords = [‘vote’, ‘election’, ‘democrat’, ‘republican’];
const containsPolitical = politicalKeywords.some(keyword =>
response.toLowerCase().includes(keyword)
);

if (containsPolitical && response.toLowerCase().includes(‘should vote’)) {
enforcedResponse += ‘\n\nPOLITICAL NEUTRALITY: I cannot tell you who to vote for. Research independently and decide for yourself.’;
}

// SITE MONKEYS ENFORCEMENT
if (mode === ‘site_monkeys’) {
if (vaultStatus === ‘loaded’ && !response.toLowerCase().includes(‘vault’) && vaultContent.length > 1000) {
enforcedResponse += ‘\n\nVAULT ENFORCEMENT: Response generated using Site Monkeys business intelligence vault.’;
}

```
const priceMatches = response.match(/\$(\d+)/g);
if (priceMatches) {
  const prices = priceMatches.map(match => parseInt(match.replace('$', '')));
  if (prices.some(price => price < 697)) {
    enforcedResponse += '\n\nPRICING ENFORCEMENT: All Site Monkeys services maintain premium pricing standards starting at $697.';
  }
}
```

}

return enforcedResponse;
}

function determinePersonality(message, mode) {
const analyticalKeywords = [‘analyze’, ‘data’, ‘risk’, ‘technical’, ‘facts’, ‘evidence’];
const creativeKeywords = [‘strategy’, ‘optimize’, ‘creative’, ‘improve’, ‘design’, ‘solution’];

const lowerMessage = message.toLowerCase();
const analyticalScore = analyticalKeywords.reduce((score, keyword) => score + (lowerMessage.includes(keyword) ? 1 : 0), 0);
const creativeScore = creativeKeywords.reduce((score, keyword) => score + (lowerMessage.includes(keyword) ? 1 : 0), 0);

return creativeScore > analyticalScore ? ‘roxy’ : ‘eli’;
}

function containsFactualClaims(response) {
const factualIndicators = [‘studies show’, ‘research indicates’, ‘data reveals’, ‘according to’, ‘statistics show’, ‘evidence suggests’, ‘analysis reveals’];
return factualIndicators.some(indicator => response.toLowerCase().includes(indicator));
}

function extractAssumptions(response) {
const assumptions = [];
if (response.includes(‘ASSUMPTIONS:’)) assumptions.push(‘explicit_assumptions_listed’);
if (response.includes(‘assume’) || response.includes(‘assuming’)) assumptions.push(‘implicit_assumptions_detected’);
if (response.includes(‘SPECULATION:’) || response.includes(‘HYPOTHESIS:’)) assumptions.push(‘speculation_marked’);
return assumptions;
}

function calculateAssumptionHealth(response) {
let score = 100;

if (!response.includes(‘CONFIDENCE:’)) score -= 20;
if (response.includes(‘probably’) || response.includes(‘likely’)) score -= 15;
if (!response.includes(‘I do not know’) && response.length < 100) score -= 10;
if (response.includes(‘INSUFFICIENT DATA’)) score += 10;
if (/CONFIDENCE:\s*(High|Medium|Low|Unknown)/i.test(response)) score += 5;

return Math.max(score, 0);
}