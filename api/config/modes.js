...
export function applyPoliticalGuardrails(response, message) {
  ...
  const hasPoliticalContent = POLITICAL_TRIGGERS.some(
    (trigger) =>
...
export function detectPressurePatterns(message, conversationHistory) {
  ...
  if (totalPressure >= 3) {
    return {
      level: 'critical',
      response:
        '🛑 PRESSURE DETECTED: Critical coercion patterns detected. System integrity protocols engaged. I cannot proceed with analysis under these conditions.',
      block: true,
    };
...
export function calculateDriftScore(overrides) {
  ...
  const timePenalty =
    Math.max(0, (Date.now() - SYSTEM_INTEGRITY.last_reset) / (1000 * 60 * 60 * 24) - 7) * 2; // Penalty after 7 days
